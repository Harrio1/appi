@echo off
echo ===== ПЕРЕЗАПУСК СЕРВЕРА И ПРИЛОЖЕНИЙ =====
...
start "API Server" cmd /k "node server.mjs --host 0.0.0.0"
...
cd ..\mobile-app
start "Mobile App" cmd /k "npm run start"

echo.
echo ===== ПЕРЕЗАПУСК ЗАВЕРШЕН ===== 

echo Для проверки CORS выполните:
echo   cd mobile-app
echo   node src\test-cors.js 